---
name: code-reviewer
description: General-purpose code review fallback. Use ONLY when no language-specific reviewer fits (mixed-language PR, generated code, config files). For C# use csharp-reviewer, for React use react-reviewer, for TypeScript use typescript-reviewer, for security use security-reviewer. This agent has the best false-positive filtering logic — good as a secondary reviewer on any language.
