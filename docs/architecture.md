...
### Real-time (SignalR)
- `AiSalesCoach.Api` hoster SignalR hub: `/hubs/coaching`
- Hub pusher hints fra API → Desktop/Extension efter hver coaching-chunk
- JWT Bearer-auth verificeres ved hub-tilslutning
- Session-ejerskab verificeres inden `AddToGroupAsync` (SignalR group = sessionId)

## Domain-Driven Design

| DDD-koncept | AiSalesCoach-implementering |
|-------------|----------------------------|
| **Aggregate Root** | `Session` (ejer `TranscriptLines`, `Hints`, `HintFeedback`) |
| **Aggregate Root** | `MeetingFile` (ejer `Transcript`, `TranscriptChunks`, `Analysis`) |
| **Entity** | `User`, `Organization`, `Framework`, `Deal` |
| **Value Object** | `SessionStatus`, `HintType`, `CoverageScore`, `HintUrgency` |
| **Domain Event** | `HintGeneratedEvent`, `SessionEndedEvent`, `AnalysisCompletedEvent` |
| **Repository Interface** | `ISessionRepository`, `IFrameworkRepository` (defineret i Domain) |
| **Bounded Context** | Real-time coaching (Sessions) vs. post-call analyse (MeetingFiles) |

Ubiquitous Language: brug altid `Session` (ikke `Call`), `Hint` (ikke `Suggestion`), `FrameworkRule` (ikke `Rule`), `TranscriptChunk` (ikke `Segment`).
